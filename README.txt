...
This repository does not commit release archives. A release zip earns its
place when the download runs as-is, and this one would not: an extracted tree
still needs a docker compose build, or a virtualenv and pip install, before
anything answers a request.

| | |
| --- | --- |
| Docker | docker compose up -d from a clone — the way the service is meant to run. |
| Source | clone the repo, or take GitHub's source archive for any tag (https://github.com/evomedia-net/evo.locate/tags). |
...
GitHub Release, or for an air-gapped copy:
    ...
    python scripts/release.py --verify   # check every zip you have built

What goes in is whatever git tracks, minus .github/ — so the archive is
exactly the reviewed source, and a new file cannot be left out by forgetting a
list. Entries are sorted and stamped with a fixed timestamp, so two builds of
the same commit produce the same bytes.

It writes a .sha256 beside the zip and a CHECKSUMS.txt inside it, and
refuses to build an archive whose name would not describe its contents:

- the tree is dirty — the zip is built from working-tree files, so
  uncommitted edits would ship inside it;
...
and neither refusal yields to it. --allow-mismatch overrides both, loudly.
