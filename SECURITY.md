...
This repository does not commit release archives. `scripts/release.py` builds
one on demand, with a `.sha256` beside the zip and a `CHECKSUMS.txt` inside.

Both are **integrity checks, not signatures** — the manifest travels in the
